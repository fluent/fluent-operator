...
// Kubernetes clusterfilter allows to enrich your log files with Kubernetes metadata.
type Kubernetes struct {
	...
	// When the source records comes from Tail clusterinput plugin,
	...
	// When Merge_Log is enabled, the clusterfilter tries to assume the log field from the incoming message is a JSON string message
	...
	// Optional clusterparser name to specify how to parse the data contained in the log key. Recommended use is for developers or testing only.
	...
	// When enabled, the clusterfilter reads logs coming in Journald format.
	...
	// The clusterparser must be registered in a parsers file (refer to clusterparser clusterfilter-kube-test as an example).
