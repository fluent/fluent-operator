...
// The Tail clusterinput plugin allows to monitor one or several text files.
...
type Tail struct {
	...
	// Specify the name of a clusterparser to interpret the entry as a structured message.
	Parser string `json:"clusterparser,omitempty"`
	// When a message is unstructured (no clusterparser applied), it's appended as a string under the key name log.
	...
	// Name of the clusterparser that matchs the beginning of a multiline message.
	// Note that the regular expression defined in the clusterparser must include a group name (named capture)
	ParserFirstline string `json:"parserFirstline,omitempty"`
	// Optional-extra clusterparser to interpret and structure multiline entries.
	// This option can be used to define multiple parsers.
	ParserN []string `json:"parserN,omitempty"`
	// If enabled, the plugin will recombine split Docker log lines before passing them to any clusterparser as configured above.
