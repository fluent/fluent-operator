...
// The Systemd clusterinput plugin allows to collect log messages from the Journald daemon on Linux environments.
type Systemd struct {
	...
	// The Systemd_Filter option can be specified multiple times in the clusterinput section to apply multiple filters as required.
	SystemdFilter []string `json:"systemdFilter,omitempty"`
	// Define the clusterfilter type when Systemd_Filter is specified multiple times. Allowed values are And and Or.
