...
var filtersAliasExpected = `[Filter]
    Name    modify
    Match    logs.foo.bar
    Alias    logs.foo.bar.alias
    Condition    Key_value_equals    kve0    kvev0
    Condition    Key_value_equals    kve1    kvev1
    Condition    Key_value_equals    kve2    kvev2
    Condition    Key_does_not_exist    kdn0    kdnv0
...
func TestAliasClusterFilterList_Load(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	for i < 5 {
		clusterFilters, err := filters.Load(sl)
		t.Log(filtersAliasExpected)
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(clusterFilters).To(Equal(filtersAliasExpected))
