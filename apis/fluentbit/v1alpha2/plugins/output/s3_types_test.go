...
func TestOutput_S3_Params(t *testing.T) {
	...
	s3 := S3{
		...
		StaticFilePath:           new(false),
		UsePutObject:             new(false),
		RoleArn:                  "role",
		Endpoint:                 "endpoint",
		...
		SendContentMd5:           new(false),
		AutoRetryRequests:        new(true),
		LogKey:                   "log_key",
		PreserveDataOrdering:     new(true),
		StorageClass:             "storage_class",
		RetryLimit:               new(int32(1)),
		ExternalId:               "external_id",
		Profile:                  "my-profile",
		Workers:                  new(int32(1)),
		Networking: &plugins.Networking{
			ConnectTimeout:         new(int32(10)),
			ConnectTimeoutLogError: new(true),
			DNSMode:                new("TCP"),
			DNSPreferIPv4:          new(false),
			DNSPreferIPv6:          new(false),
			IOTimeout:              new(int32(0)),
			KeepaliveMaxRecycle:    new(int32(2000)),
			MaxWorkerConnections:   new(int32(0)),
			ProxyEnvIgnore:         new(false),
			SourceAddress:          new("127.0.0.1"),
			TCPKeepaliveInterval:   new(int32(-1)),
			TCPKeepaliveProbes:     new(int32(-1)),
			TCPKeepaliveTime:       new(int32(-1)),
		},
	}
