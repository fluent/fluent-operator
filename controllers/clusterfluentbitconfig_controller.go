...
func (r *ClusterFluentBitConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	var cfgs clusterfluentbit.ClusterFluentBitConfigList
	...
	for _, cfg := range cfgs.Items {
		...
		var inputs clusterfluentbit.ClusterInputList
		...
		var filters clusterfluentbit.ClusterFilterList
		...
		var outputs clusterfluentbit.ClusterOutputList
		...
		var parsers clusterfluentbit.ClusterParserList
...
func (r *ClusterFluentBitConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &corev1.Secret{}, ownerKey, func(rawObj client.Object) []string {
		...
		// Make sure it's a ClusterFluentBitConfig. If so, return it.
		if owner.APIVersion != clusterapiGVStr || owner.Kind != "ClusterFluentBitConfig" {
			return nil
		}
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Owns(&corev1.Secret{}).
		Watches(&source.Kind{Type: &clusterfluentbit.ClusterInput{}}, &handler.EnqueueRequestForObject{}).
		Watches(&source.Kind{Type: &clusterfluentbit.ClusterFilter{}}, &handler.EnqueueRequestForObject{}).
		Watches(&source.Kind{Type: &clusterfluentbit.ClusterOutput{}}, &handler.EnqueueRequestForObject{}).
		Watches(&source.Kind{Type: &clusterfluentbit.ClusterParser{}}, &handler.EnqueueRequestForObject{}).
		Complete(r)
}
